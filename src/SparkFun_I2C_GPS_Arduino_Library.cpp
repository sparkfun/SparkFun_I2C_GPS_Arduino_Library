/*
  This is a library written for the MediaTek MT3333 based GPS module with I2C
  firmware specially loaded. SparkFun sells these at its website:
  www.sparkfun.com
  ...
  This library handles the pulling in of data over I2C. We recommend a parsing
  engine such as TinyGPS.
...
bool I2CGPS::begin(I2C &wirePort, uint32_t i2cSpeed) {
  ...
  if (0 == _i2cPort->read(
               MT333x_ADDR << 1 | 1, (char *)&dummy,
               sizeof(dummy))) // might fail if current is unstable on GPIO pins
    return (true);
  else
    return (false);
}
  ...
  for (uint8_t x = 0; x < MAX_PACKET_SIZE; x++) {
    ...
    if (incoming != 0x0A) {
      ...
      if (_printDebug == true && _head == _tail) {
        ...
        #elif defined(__MBED__)
        const char response[16] = "Buffer overrun\n";
        #if MBED_MAJOR_VERSION == 6
        _debugSerial->write(response, sizeof(response));
        #elif MBED_MAJOR_VERSION == 5
        _debugSerial->printf(response);
        #endif
    #endif
      }
    }
...
void I2CGPS::enableDebugging(UnbufferedSerial &debugPort) {
#elif MBED_MAJOR_VERSION == 5
void I2CGPS::enableDebugging(Serial &debugPort) {
#endif
...
// The input buffer on the MTK is 255 bytes. Caller must keep strings shorter
// than 255 bytes Any time you end transmission you must give the module 10ms to
// process bytes
#if defined(ARDUINO)
boolean I2CGPS::sendMTKpacket(String command) {
  ...
  for (uint8_t chunk = 0; chunk < 7;
       chunk++) // 8 chunks * 32 = 256 bytes total so we need to shave one
  {
    _i2cPort->beginTransmission(MT333x_ADDR);
  ...
  // Send final 31 bytes
  if (command.length() >
      (7 * 32)) // Do we have more than 224 bytes? Then send last 31
  {
    _i2cPort->beginTransmission(MT333x_ADDR);
...
bool I2CGPS::sendMTKpacket(string command) {
  if (command.length() > 255) {
    if (_printDebug == true) {
      const char response[28] = "Command message too long!\n";
#if MBED_MAJOR_VERSION == 6
      _debugSerial->write(response, sizeof(response));
#elif MBED_MAJOR_VERSION == 5
      _debugSerial->printf(response);
...
// Given a string of characters, XOR them all together and return CRC in string
// form
#if defined(ARDUINO)
...
#endif
  return (sendMTKpacket(
      command)); // Send process is the same, re-named to ease user's minds
}
...
string I2CGPS::createPGCMDpacket(uint16_t packetType, string dataField) {
  ...
  configSentence +=
      calcCRCforMTK(configSentence); // Attach CRC - uses the same crc as PTMK
